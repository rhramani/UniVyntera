...
import {
  ...
  getAllType,
  updateType,
} from "../../../redux/actions/Master/TaskManagementMaster/TaskType.action";
...
const TaskType = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Task Type");

  useEffect(() => {
    if (canRead) {
      fetchTypes(currentPage, itemsPerPage, search);
    } else {
      setType([]);
      setTotalRecords(0);
      setTotalPages(0);
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (values.id && canUpdate) {
          res = await dispatch(updateType(values, values.id));
          if (res?.data?.code === 200) {
            toast.success("Type updated successfully");
          }
        } else if (!values.id && canCreate) {
          res = await dispatch(createType(values));
        ...
        if (canRead) {
          setCurrentPage(1);
          fetchTypes(1, itemsPerPage, search);
        }
      } catch (error) {
  ...
  const handleDelete = async (item) => {
    if (canDelete) {
      try {
        toast.dismiss();
        const res = await dispatch(deleteType(item._id));
        if (res?.data?.code === 200) {
  ...
  return (
    <>
      <Pageheader
        mainheading="Task Type"
        parentfolder="Task"
        activepage="Task Type"
      />
