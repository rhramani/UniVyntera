...
const TaskCategory = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Task Category");

  useEffect(() => {
    if (canRead) {
      fetchCategorys(currentPage, itemsPerPage, search);
    } else {
      setCategory([]);
      setTotalRecords(0);
      setTotalPages(0);
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (values.id && canUpdate) {
          res = await dispatch(updateCategory(values, values.id));
          if (res?.data?.code === 200) {
            toast.success("Category updated successfully");
          }
        } else if (!values.id && canCreate) {
          res = await dispatch(createCategory(values));
        ...
        if (canRead) {
          setCurrentPage(1);
          fetchCategorys(1, itemsPerPage, search);
        }
      } catch (error) {
