...
const MainPlan = () => {
  ...
  const [allMainPlans, setAllMainPlans] = useState([]);
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Main Plan");

  useEffect(() => {
    if (canRead) {
      fetchMainPlans(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchMainPlans(1, newItemsPerPage, search);
    }
  };
  ...
  const fetchMainPlans = async (
    ...
    try {
      const res = await dispatch(getAllMainPlan(page, limit, search));
      const responseData = res?.data?.data;
      setAllMainPlans(responseData?.data || []);
      setTotalPages(responseData?.totalPages || 0);
      setTotalRecords(responseData?.totalRecords || 0);
    } catch (error) {
      console.error("Error fetching main plan names:", error);
      setAllMainPlans([]);
      setTotalPages(0);
    }
  };
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("Main Plan name is required"),
    }),
      ...
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateMainPlan({ name: values?.name }, values?.id)
          );
          if (res?.status === 200) {
            toast.success("Main plan name updated successfully");
            setHighlightForm(false);
            resetForm();
          }
        } else if (!values.id && canCreate) {
          const res = await dispatch(createMainPlan({ name: values?.name }));
          if (res?.status == 201) {
            toast.success("Main plan name added successfully");
            setHighlightForm(false);
            resetForm();
          }
        }
        ...
        toast.dismiss();
        toast.error("Main plan name already exists..");
        resetForm();
      }
    },
  });

  const handleEdit = (mainPlan) => {
    if (canUpdate) {
      formik.setFieldValue("name", mainPlan?.name);
      formik.setFieldValue("id", mainPlan?._id);
  ...
  const handleDelete = async (mainPlan) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteMainPlan(mainPlan?._id));
      if (res?.data?.code === 200) {
        toast.success("Main Plan deleted successfully");
      }
      const updatedPage =
        allMainPlans.length === 1 && currentPage > 1
          ? currentPage - 1
          : currentPage;
      ...
      toast.error("Failed to delete the mainPlan.");
    }
  };
  useEffect(() => {
