...
import { createExpenseType, deleteExpenseType, getAllExpenseType, updateExpenseType } from "../../redux/actions/Master/ExpenseType.action";
...
const ExpenseType = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Expense Type");

  useEffect(() => {
    if (canRead) {
      fetchExpenseType(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchExpenseType(1, newItemsPerPage, search);
    }
  };
  ...
  const fetchExpenseType = async (
    ...
    try {
      const res = await dispatch(getAllExpenseType(page, limit, search));
      const responseData = res?.data?.data;
      setAllExpenseType(responseData?.data || []);
      ...
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateExpenseType({ name: values?.name }, values?.id)
          );
          ...
          const res = await dispatch(
            createExpenseType({ name: values?.name })
          );
  ...
  const handleEdit = (name) => {
    if (canUpdate) {
      ...
      formik.setFieldValue("id", name?._id);
  ...
  const handleDelete = async (name) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteExpenseType(name?._id));
      if (res?.data?.code === 200) {
        toast.success("Expense Type deleted successfully");
      }
      const updatedPage =
        allExpenseType.length === 1 && currentPage > 1
          ? currentPage - 1
          : currentPage;
      ...
      toast.error("Failed to delete the expense type.");
    }
  };
  useEffect(() => {
  ...
  return (
    <>
      <Pageheader
        mainheading="Expense Type"
        parentfolder="Master"
        activepage="Expense Type"
      />
          ...
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              ...
              <div className="card-title">
                {highlightForm ? "Expense Type" : "Expense Type"}
              </div>
            </div> */}
            </Card.Header>
            <Card.Body>
              ...
              {totalPages > 1 && allExpenseType.length > 0 && (
