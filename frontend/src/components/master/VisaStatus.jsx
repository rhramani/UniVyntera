...
const VisaStatus = () => {
  ...
  useEffect(() => {
    if (canRead) {
      fetchVisaStatus(search);
    }
  }, [search]);

  const fetchVisaStatus = async (search) => {
    try {
      const res = await dispatch(getAllVisaStatus(search));
      if (res?.status === 200) {
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(updateVisaStatus(payload, values.id));
          if (res?.status === 200) {
            toast.success("Visa Status updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createVisaStatus(payload));
          if (res?.status === 201) {
            toast.success("Visa Status created successfully");
          }
        }
  ...
  const handleDelete = async (item) => {
    if (canDelete) {
      try {
        const res = await dispatch(deleteVisaStatus(item._id));
        if (res?.status === 200) {
          toast.success("Visa Status deleted successfully");
          if (canRead) {
            fetchVisaStatus(search);
          }
        }
          ...
          <Card.Body>
            ...
            <Modal show={showUploadModal} onHide={handleCloseUploadModal}>
              <Modal.Header className="form-main-heading">
                <Modal.Title>
                  {formik.values.id ? "Update Visa Status" : "Add Visa Status"}
                </Modal.Title>
                <AiOutlineClose
                  size={20}
