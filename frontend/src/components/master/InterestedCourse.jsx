...
const InterestedCourse = () => {
  ...
  useEffect(() => {
    if (canRead) {
      fetchStudentStatuses(search);
    }
  }, [search]);

  const fetchStudentStatuses = async (search) => {
    try {
      const res = await dispatch(getAllInterestedCourseStatus(search));
      if (res?.status === 200) {
      ...
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateInterestedCourseStatus(payload, values.id)
          );
          if (res?.status === 200) {
            toast.success("Interested Course Status updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createInterestedCourseStatus(payload));
          if (res?.status === 201) {
            toast.success("Interested Course Status created successfully");
          }
        }
  ...
  const handleDelete = async (item) => {
    if (canDelete) {
      try {
        const res = await dispatch(deleteInterestedCourseStatus(item._id));
        if (res?.status === 200) {
          toast.success("Interested Course Status deleted successfully");
          if (canRead) {
            fetchStudentStatuses(search);
          }
        }
