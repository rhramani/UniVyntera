...
const VisitorType = () => {
  ...
  const [allVisitorType, setAllVisitorType] = useState([]);
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Visitor Type");

  useEffect(() => {
    if (canRead) {
      fetchVisitorType(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchVisitorType(1, newItemsPerPage, search);
    }
  };
  ...
  const fetchVisitorType = async (
    ...
    try {
      const res = await dispatch(getAllVisitorType(page, limit, search));
      const responseData = res?.data?.data;
      setAllVisitorType(responseData?.data || []);
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("Visitor Type is required"),
    }),
      ...
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateVisitorType({ name: values?.name }, values?.id)
          );
          if (res?.data?.code === 200) {
            toast.success("Visitor Type updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createVisitorType({ name: values?.name }));
  ...
  const handleEdit = (visitorType) => {
    if (canUpdate) {
      formik.setFieldValue("name", visitorType?.name);
      formik.setFieldValue("id", visitorType?._id);
      setHighlightForm(true);
    }
  };

  const handleDelete = async (visitorType) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteVisitorType(visitorType?._id));
      if (res?.data?.code === 200) {
        toast.success("Visitor Type deleted successfully");
      }
      const updatedPage =
        allVisitorType.length === 1 && currentPage > 1
          ? currentPage - 1
          : currentPage;
      setCurrentPage(updatedPage);
      if (canRead) {
        fetchVisitorType(updatedPage, itemsPerPage, search);
      }
    } catch (error) {
      console.log("Error", error);
      toast.error("Failed to delete the Visitor Type.");
    }
  };
  useEffect(() => {
              ...
              {(canCreate || (canUpdate && formik.values.id)) && (
                <div className="form_left_section">
                  <div className="form-group">
                    <Form.Label className="form-label">Visitor Type</Form.Label>
                    <Form.Control
                      type="text"
                      name="name"
