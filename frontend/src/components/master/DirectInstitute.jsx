...
const DirectInstitute = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Direct Institute");
  ...
  const instituteSections = [
    {
      ...
      fields: [
        ...
        { label: "Institute Name", key: "instituteName" },
        {
          label: "Campus",
          key: "campus",
          render: (item) => item?.campus || "N/A",
        },
        { label: "Postal Address", key: "postalAddress" },
        {
          label: "Recruitment Territory Rights",
          key: "recruitmentTerritoryRights",
        },
        { label: "Type Of Association", key: "typeOfAssociation" },
      ],
    },
                  ...
                  <tbody>
                    {list.map((p, i) => (
                      <tr key={i}>
                        ...
                        <td style={{ padding: "6px" }}>{p.designation}</td>
...

    {
      ...
      fields: [
        { label: "Admission Type", key: "admissionType" },
        {
          ...
          render: (item) => `${item?.commissionPercentage || "N/A"}%`,
        },
        { label: "Commission Period", key: "commissionPeriod" },
        { label: "Commission Structure", key: "commissionStructure" },
      ],
    },
      ...
      fields: [
        {
          ...
          render: (item) =>
            item?.agreementStartDate
              ? new Date(item.agreementStartDate).toLocaleDateString("en-GB")
              : "N/A",
        },
        {
          ...
          render: (item) =>
            item?.agreementEndDate
              ? new Date(item.agreementEndDate).toLocaleDateString("en-GB")
              : "N/A",
        },
        { label: "Agreement Status", key: "agreementStatus" },

        {
          ...
          render: (item) =>
            item?.agreementDoc ? (
              <a
                href={`${REACT_APP_API_URL}/${item.agreementDoc}`}
                target="_blank"
...

        {
          ...
          render: (item) =>
            item?.brochure ? (
              <a
                href={`${REACT_APP_API_URL}/${item.brochure}`}
                target="_blank"
                rel="noopener noreferrer"
              >
                View Brochure
...

    {
      ...
      fields: [
        { label: "Bachelors", key: "bachelors" },
        { label: "Masters", key: "masters" },
        { label: "Post Graduate Diploma", key: "postGraduateDiploma" },
        { label: "Diploma", key: "diploma" },
        { label: "Foundation Year", key: "foundationYear" },
...

    {
      ...
      fields: [
        ...
        },
        {
          ...
          render: (item) =>
            item?.portal ? (
              <a href={item.portal} target="_blank" rel="noopener noreferrer">
                {item.portal}
              </a>
            ) : (
              "N/A"
...

    {
      ...
      fields: [{ label: "Other Info", key: "otherInfo" }],
    },
    {
      title: "Images",
      fields: [
  ...
  const statusFilterOptions = [
    { value: "notExpired", label: "Not Expired" },
    { value: "expired", label: "Expired" },
    { value: "expiringInMonth", label: "Expiring In Month" },
  ];
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchAllInstitute(1, newItemsPerPage, search, statusFilter);
    }
  };
  ...
  useEffect(() => {
    if (canRead) {
      fetchAllInstitute(currentPage, itemsPerPage, search, statusFilter);
      fetchCountries();
      fetchCampuses();
    } else {
      setInstituteList([]);
  ...
  }, [currentPage, search, statusFilter, canRead]);
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      masters: Yup.string(),
      postGraduateDiploma: Yup.string(),
      diploma: Yup.string(),
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        //   );
        // }
        const olTAT = formattedValues?.olTATPeriod || {};
        payload.append("olTATPeriod[value]", olTAT?.value ?? "");
        payload.append("olTATPeriod[unit]", olTAT?.unit ?? "");
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(updateDirectInstitute(payload, values.id));
          if (res?.data?.code === 200) {
            toast.success("Institute updated successfully");
            handleClose();
          }
        } else if (canCreate) {
          const res = await dispatch(createDirectInstitute(payload));
    ...
    if (canUpdate) {
      try {
        ...
        formik.setValues({
          ...
          olTATPeriod: (() => {
            try {
              if (!item?.olTATPeriod) return { value: "", unit: "" };
              if (typeof item.olTATPeriod === "string") {
                const parsed = JSON.parse(item.olTATPeriod);
                return {
                  value: parsed?.value ?? "",
                  unit: parsed?.unit ?? "",
                };
              ...
              return {
                value: item.olTATPeriod?.value ?? "",
                unit: item.olTATPeriod?.unit ?? "",
              };
            } catch {
              return { value: "", unit: "" };
            }
          ...
          postGraduateDiploma: item.postGraduateDiploma || "",
  ...
  const handleDelete = async (item) => {
    if (canDelete) {
      try {
        toast.dismiss();
        const res = await dispatch(deleteDirectInstitute(item._id));
        if (res?.data?.code === 200) {
  ...
  const columns = [
    ...
    //     const endDate = item?.agreementEndDate
    //       ? new Date(item.agreementEndDate)
    //       : null;
    ...
    //           The institute’s agreement date ends today!
    //         </span>
    //       );
    //     } else if (endDate && endDate < today) {
    //       return (
    //         <span style={{ color: "orange" }}>
    //           Agreement expired on {endDate.toLocaleDateString()}
    //         </span>
    //       );
    //     } else if (endDate) {
    //       return (
    ...
    // },
    {
      ...
      render: (item) => {
        ...
        if (daysLeft < 0) {
          return (
            <span style={{ color: "red", fontWeight: "bold" }}>
              Expired on {formatDDMMYYYY(endDate)}
            </span>
          );
        }
        ...
        return (
          <span style={{ color: "green", fontWeight: "bold" }}>
            Valid till {formatDDMMYYYY(endDate)}
          </span>
        );
      },
    },
...
export default DirectInstitute;
