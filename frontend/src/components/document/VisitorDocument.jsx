...
import { getAllVisitorDocumentList } from "../../redux/actions/Document/VisitorDocumentList.action";
...
const VisitorDocument = () => {
  ...
  const [allTouristDocument, setAllTouristDocument] = useState([]);
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Visitor Document");
  ...
  useEffect(() => {
    if (canRead) {
      fetchVisitorDocuments(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchVisitorDocuments(1, newItemsPerPage, search);
    }
  };

  const fetchVisitorDocuments = async (
    page = 1,
    limit = itemsPerPage,
    search = ""
  ) => {
    try {
      const res = await dispatch(getAllVisitorDocument(page, limit, search));
      const responseData = res?.data?.data;
      setAllTouristDocument(responseData?.data || []);
      setTotalPages(responseData?.totalPages || 0);
      setTotalRecords(responseData?.totalRecords || 0);

      const assignedDocs = {};
      responseData?.data?.forEach((touristDoc) => {
        touristDoc.documents.forEach(({ type, documentList }) => {
          if (type?._id) {
  ...
  const fetchDocumentTypes = async (page = 1, limit = 100, search = "") => {
    try {
      const res = await dispatch(
        getAllVisitorDocumentType(page, limit, search)
      );
      const responseData = res?.data?.data;
  ...
  const fetchDocumentLists = async () => {
    try {
      ...
      for (const type of allDocumentTypes) {
        const res = await dispatch(
          getAllVisitorDocumentList(1, 10000, "", type._id)
        );
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateVisitorDocument(payload, values?.id)
          );
          if (res?.data?.code === 200) {
            toast.success("Tourist document updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createVisitorDocument(payload));
          if (res?.data?.code === 201) {
            toast.success("Tourist document added successfully");
          }
        }
        resetForm();
        setSelectedDocuments({});
  ...
  const handleDelete = async (countryDocument) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteVisitorDocument(countryDocument?._id));
      if (res?.data?.code === 200) {
        toast.success("Tourist document deleted successfully");
      }
      const updatedPage =
        allTouristDocument?.length === 1 && currentPage > 1
          ? currentPage - 1
          : currentPage;
      setCurrentPage(updatedPage);
      if (canRead) {
        fetchVisitorDocuments(updatedPage, itemsPerPage, search);
      }
    } catch (error) {
      console.log("Error", error);
      toast.error("Failed to delete the Tourist document.");
    }
  };
...
export default VisitorDocument;
