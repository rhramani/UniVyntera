...
const documentType = [
  ...
  { label: "Education Document", value: "Education Document" },
  { label: "Financial Documents", value: "Financial Documents" },
  { label: "Work Document", value: "Work Document" },
  { label: "Other Documents", value: "Other Documents" },
  {
    label: "After University Admission Document",
    value: "After University Admission Document",
  },
];
...
const DocumentList = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Assign Document List");
  useEffect(() => {
  ...
  const fetchDocumentLists = async (
    ...
    try {
      const res = await dispatch(getAllDocumentList(page, limit, search));
  ...
  const fetchDocumentType = async () => {
    try {
      const res = await dispatch(getAllDocumentType(1, 100));
      const responseData = res?.data?.data;
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(updateDocumentList(values, values?.id));
          if (res?.data?.code === 200) {
            toast.success("Document List updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createDocumentList(values));
          if (res?.data?.code == 201) {
            toast.success("Document List added successfully");
          }
        }
        resetForm();
        if (canRead) {
        ...
        toast.dismiss();
        toast.error("Document List already exists..");
        resetForm();
      }
    },
  });
  ...
  const handleDelete = async (documentList) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteDocumentList(documentList?._id));
      if (res?.data?.code === 200) {
      ...
      toast.error("Failed to delete the Document List.");
    }
  };
  useEffect(() => {
