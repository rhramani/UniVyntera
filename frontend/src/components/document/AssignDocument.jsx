...
} from "../../redux/actions/Document/AssignDocument.action";
...
const AssignDocument = () => {
  ...
  const [allCountryDocument, setAllCountryDocument] = useState([]);
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Assign Document");
  ...
  useEffect(() => {
    if (canRead) {
      fetchCountryDocuments(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchCountryDocuments(1, newItemsPerPage, search);
    }
  };

  const fetchCountryDocuments = async (
    page = 1,
    limit = itemsPerPage,
    search = ""
  ) => {
    try {
      const res = await dispatch(getAllCountryDocument(page, limit, search));
      const responseData = res?.data?.data;
      setAllCountryDocument(responseData?.data || []);
      setTotalPages(responseData?.totalPages || 0);
      setTotalRecords(responseData?.totalRecords || 0);

      const assignedDocs = {};
      responseData?.data?.forEach((countryDoc) => {
        countryDoc.documents.forEach(({ type, documentList }) => {
          if (type?._id) {
      ...
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateCountryDocument(payload, values?.id)
          );
          if (res?.data?.code === 200) {
            toast.success("Country document updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createCountryDocument(payload));
          if (res?.data?.code === 201) {
            toast.success("Country document added successfully");
          }
        }
        resetForm();
        setSelectedDocuments({});
  ...
  const handleDelete = async (countryDocument) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteCountryDocument(countryDocument?._id));
      if (res?.data?.code === 200) {
        toast.success("Country document deleted successfully");
      }
      const updatedPage =
        allCountryDocument.length === 1 && currentPage > 1
          ? currentPage - 1
          : currentPage;
      setCurrentPage(updatedPage);
      if (canRead) {
        fetchCountryDocuments(updatedPage, itemsPerPage, search);
      }
    } catch (error) {
      console.log("Error", error);
      toast.error("Failed to delete the Country document.");
    }
  };
