...
const VisitorDocumentType = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("Visitor Document Type is required"),
    }),
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateVisitorDocumentType({ name: values?.name }, values?.id)
          );
          if (res?.data?.code === 200) {
            toast.success("Visitor Document Type updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(
            createVisitorDocumentType({ name: values?.name })
          );
          if (res?.data?.code == 201) {
            toast.success("Visitor Document Type added successfully");
          }
        }
        resetForm();
        if (canRead) {
  ...
  const handleDelete = async (documentType) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteVisitorDocumentType(documentType?._id));
      if (res?.data?.code === 200) {
        toast.success("Visitor Document Type deleted successfully");
      }
      const updatedPage =
