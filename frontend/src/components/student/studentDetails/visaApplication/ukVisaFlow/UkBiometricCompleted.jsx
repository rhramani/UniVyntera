...
const UkBiometricCompleted = ({ id }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [applicationData, setApplicationData] = useState(null);
  const [selectedDocsIds, setSelectedDocsIds] = useState([]);
  const [selectedDocumentNames, setSelectedDocumentNames] = useState([]);

  const [showBiometricDateCalendar, setShowBiometricDateCalendar] =
    useState(false);
  const biometricDateRef = useRef(null);

  const dispatch = useDispatch();
  const userRole = decryptData(localStorage.getItem("role"));
  const documentTypes = ["Biometric Slip"];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        biometricDateRef.current &&
        !biometricDateRef.current.contains(event.target)
      ) {
        setShowBiometricDateCalendar(false);
      }
    };
  ...
  const formik = useFormik({
    initialValues: {
      completed:
        applicationData?.visaApplicationDetails?.biometricCompletion
          ?.completed || false,
      biometricDate: applicationData?.visaApplicationDetails
        ?.biometricCompletion?.biometricDate
        ? toISODate(
            parseDate(
              applicationData.visaApplicationDetails.biometricCompletion
                .biometricDate
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        const jsonData = {
          visaApplicationDetails: {
            biometricCompletion: {
              ...
              biometricDate: values.biometricDate,
            },
          },
        };

        if (
          oldData.completed !== values.completed ||
          oldData.biometricDate !== values.biometricDate
        ) {
          hasJsonChanges = true;
        }

        let formData = null;
        if (values.biometricSlipUpload) {
          hasFileUpload = true;
          formData = new FormData();
          formData.append("uploadedDocument", values.biometricSlipUpload);
          formData.append("customDocumentName", "Biometric Slip");
          formData.append(
            "ref_module",
        ...
        await fetchData();
        toast.success("Biometric completion details updated successfully!");
        resetForm();
      } catch (error) {
        toast.error("Failed to update biometric completion details.");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleFileChange = (event) => {
    formik.setFieldValue("biometricSlipUpload", event.target.files[0]);
  };
  ...
  return (
    <div className="mb-4 my-5 p-4 bg-light rounded shadow-sm">
      ...
      <h5>Biometric Completed</h5>
      <div className="bg-white rounded mt-3 p-3">
        <Form onSubmit={formik.handleSubmit}>
          <Row>
            <Col md={6} className="mb-3">
              <Form.Group>
                <Form.Label>Biometric Completed?</Form.Label>
                <div>
                  <Form.Check
                    inline
                    type="radio"
                  ...
                  <Form.Check
                    ...
                    onChange={() => formik.setFieldValue("completed", false)}
                    className="custom-radio-border"
                    disabled={userRole === "Student" || userRole === "LeadStudent"}
                  />
                </div>
            ...
            {formik.values.completed === true && (
              <>
                <Col md={6} className="mb-3">
                  <Form.Group>
                    <Form.Label>Biometric Date</Form.Label>
                    <div
                      style={{ position: "relative" }}
                      ref={biometricDateRef}
                    >
                      <Form.Control
                        type="text"
                        placeholder="dd/mm/yyyy"
                        value={
                          formik.values.biometricDate
                            ? formatDate(parseDate(formik.values.biometricDate))
                            : ""
                        }
                        readOnly
                        onClick={() => setShowBiometricDateCalendar(true)}
                      ...
                      {showBiometricDateCalendar && (
                        ...
                        >
                          <Calendar
                            ...
                            value={
                              parseDate(formik.values.biometricDate) || null
                            }
                            locale="en-GB"
                          />
                        </div>
                ...
                <Col md={6} className="mb-3">
                  <Form.Group>
                    <Form.Label>Upload Biometric Slip</Form.Label>
                    <Form.Control
                      type="file"
                      ...
                      disabled={
                        applicationData?.uploadedDocumentDetails?.some(
                          (doc) => doc.customDocumentName === "Biometric Slip"
...
export default UkBiometricCompleted;
