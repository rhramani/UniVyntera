...
import {
  ...
  getAllDegree,
  updateDegree,
} from "../../redux/actions/Lead/Degree.action";
...
const Degree = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } = usePermissions("Degree");

  useEffect(() => {
    if (canRead) {
      fetchDegrees(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchDegrees(1, newItemsPerPage, search);
    }
  };

  const fetchDegrees = async (page = 1, limit = itemsPerPage, search = "") => {
    try {
      const res = await dispatch(getAllDegree(page, limit, search));
      const responseData = res?.data?.data;
      setAllDegree(responseData?.data || []);
      ...
      try {
        ...
        if (values.id && canUpdate) {
          const res = await dispatch(
            updateDegree({ name: values?.name }, values?.id)
          );
          if (res?.data?.code === 200) {
            toast.success("Degree updated successfully");
          }
        } else if (canCreate) {
          const res = await dispatch(createDegree({ name: values?.name }));
  ...
  const handleEdit = (degree) => {
    if (canUpdate) {
      formik.setFieldValue("name", degree?.name);
      formik.setFieldValue("id", degree?._id);
  ...
  const handleDelete = async (degree) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteDegree(degree?._id));
      if (res?.data?.code === 200) {
        toast.success("Degree deleted successfully");
      }
      const updatedPage =
        allDegree.length === 1 && currentPage > 1
          ? currentPage - 1
          : currentPage;
      setCurrentPage(updatedPage);
      if (canRead) {
        fetchDegrees(updatedPage, itemsPerPage, search);
      }
    } catch (error) {
      console.log("Error", error);
      toast.error("Failed to delete the Degree.");
    }
  };
  useEffect(() => {
          ...
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              ...
              <div className="card-title">
                {highlightForm ? "Update Degree" : "Add Degree"}
              </div>
            </div> */}
            </Card.Header>
            <Card.Body>
              ...
              {totalPages > 1 && allDegree.length > 0 && (
