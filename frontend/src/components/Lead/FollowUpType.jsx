...
const FollowUpType = () => {
  ...
  useEffect(() => {
    if (canRead) {
      fetchFollowUpTypes(currentPage, itemsPerPage, search);
    }
  }, [currentPage, search]);
  ...
  const handleItemsPerPageChange = (newItemsPerPage) => {
    ...
    setCurrentPage(1);
    if (canRead) {
      fetchFollowUpTypes(1, newItemsPerPage, search);
    }
  };

  const fetchFollowUpTypes = async (page = 1, limit = itemsPerPage, search = "") => {
    try {
      const res = await dispatch(getAllFollowUpType(page, limit, search));
  ...
  const handleEdit = (type) => {
    if (canUpdate) {
      formik.setFieldValue("name", type?.name);
      formik.setFieldValue("id", type?._id);
  ...
  const handleDelete = async (type) => {
    try {
      ...
      if (canRead) {
        fetchFollowUpTypes(updatedPage, itemsPerPage, search);
      }
    } catch (error) {
