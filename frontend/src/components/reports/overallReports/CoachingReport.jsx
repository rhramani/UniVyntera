...
import { getAllCoachingReport } from "../../../redux/actions/Report/OverallReports.action";
...
const CoachingReport = () => {
  ...
  const { canRead, canDownload } = usePermissions("Coaching Report");
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item) => item?.totalApplication || "-",
    },
    {
      label: "Achieved Score",
      key: "archivedScore",
      render: (item) => item?.archivedScore || "-",
    },
    {
      label: "Not Achieved Score",
      key: "notArchivedScore",
      render: (item) => item?.notArchivedScore || "-",
    },
  ];

  const fetchCoachingReports = async (
    page = 1,
    limit = itemsPerPage,
    search = ""
  ) => {
    try {
      ...
      setCoachingData(res?.data?.data?.data?.records || []);
  ...
  useEffect(() => {
    if (canRead) {
      fetchCoachingReports(currentPage, itemsPerPage, search);
    }
  ...
  const handleExport = async () => {
    try {
      ...
      const allCourseReports = res?.data?.data?.data?.records || [];
      ...
      toast.success("Coaching  report downloaded successfully!");
    } catch (error) {
          ...
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              <div className="w-100 d-flex flex-wrap justify-content-between">
                ...
                {coachingData?.length > 0 && canDownload && (
                  <Button
                    variant="primary"
            ...
            <Card.Body>
                ...
                >
                  ...
                  <tbody>
                    {coachingData?.length > 0 ? (
                      coachingData.filter(Boolean).map((item, index) => (
                        <tr
                          key={item._id || index}
                          className={`${
              ...
              {totalPages > 1 && coachingData.length > 0 && (
