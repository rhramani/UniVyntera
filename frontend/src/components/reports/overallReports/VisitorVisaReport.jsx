...
import { getAllVisitorVisaReport } from "../../../redux/actions/Report/OverallReports.action";

const VisitorVisaReport = () => {
  ...
  const { canRead, canDownload } = usePermissions("Visitor Visa");
  ...
  const columns = [
    ...
    },
    {
      label: "Pending Visa",
      key: "pendingVisa",
      render: (item) => item?.pendingVisa || "-",
    },
    {
      label: "Total Applications",
      key: "totalApplications",
      render: (item) => item?.totalApplications || "-",
    },
    {
      label: "Visa Achieved",
      key: "visaArchived",
      render: (item) => item?.visaArchived || "-",
    },
  ];

  const fetchVisitorVisaReports = async (
    page = 1,
    limit = itemsPerPage,
    search = ""
  ) => {
    try {
      ...
      setVisitorVisaData(res?.data?.data?.data?.data || []);
      setTotalRecords(res?.data?.data?.data?.totalRecords + 1 || 0);
      setTotalPages(res?.data?.data?.data?.totalPages || 0);
    } catch (error) {
      console.error("Error fetching Visitor Visa reports:", error);
      setVisitorVisaData([]);
    }
  };
  ...
  useEffect(() => {
    if (canRead) {
      fetchVisitorVisaReports(currentPage, itemsPerPage, search);
    }
  ...
  const handleExport = async () => {
    try {
      ...
      const allCourseReports = res?.data?.data?.data?.data || [];
      ...
      toast.success("Visitor Visa report downloaded successfully!");
    } catch (error) {
  ...
  return (
    <>
      <Pageheader
        mainheading="Visitor Visa"
        parentfolder="Reports"
        activepage="Visitor Visa"
      />
            ...
            <Card.Header className="border-bottom-0">
              <div className="w-100 d-flex flex-wrap justify-content-between">
                <div className="d-flex justify-content-between">
                  <div className="card-title">Visitor Visa Report</div>
                </div>
                ...
                {visitorVisaData?.length > 0 && canDownload && (
                  <Button
                    variant="primary"
            ...
            <Card.Body>
              ...
              <div className="table-responsive">
                ...
                >
                  ...
                  <tbody>
                    {visitorVisaData?.length > 0 ? (
                      visitorVisaData.filter(Boolean).map((item, index) => (
                        <tr
                          key={item._id || index}
                          className={`${
              ...
              {totalPages > 1 && visitorVisaData.length > 0 && (
...
export default VisitorVisaReport;
