...
import { getAllOverallReport } from "../../../redux/actions/Report/OverallReports.action";
...
const OverAllReport = () => {
  ...
  const { canRead,canDownload } = usePermissions("Overall");
  ...
  const columns = [
    ...
    { label: "Admission Applied", key: "admissionApplied" },
    { label: "Admission Confirmed", key: "admissionConfirmed" },
    { label: "Visa Approved", key: "visaApproved" },
    { label: "Visa Rejected", key: "visaRejected" },
    { label: "TT Copy Received", key: "ttCopyReceived" },
  ];

  const fetchOverallReports = async (
    page = 1,
    limit = itemsPerPage,
    search = ""
  ) => {
    try {
      const res = await dispatch(
        getAllOverallReport(page, limit, search, startDate || "", endDate || "")
      );
      setOverallData(res?.data?.data?.branches || []);
      setTotalRecords(res?.data?.data?.totalBranches + 1 || 0);
  ...
  useEffect(() => {
    if (canRead) {
      fetchOverallReports(currentPage, itemsPerPage, search);
    }
  ...
  const handleExport = async () => {
    try {
      ...
      const allCourseReports = res?.data?.data?.branches || [];
        ...
        <Col md={12} lg={12} xl={12}>
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              <div className="w-100 d-flex flex-wrap justify-content-between">
                <div className="d-flex justify-content-between">
                  <div className="card-title">Overall Report</div>
                </div>
                ...
                {overallData?.length > 0 && canDownload && (
                  <Button
                    variant="primary"
            ...
            <Card.Body>
                ...
                >
                  ...
                  <tbody>
                    {overallData?.length > 0 ? (
                      overallData.filter(Boolean).map((item, index) => (
                        <tr
                          key={item._id || index}
                          className={`${
              ...
              {totalPages > 1 && overallData.length > 0 && (
