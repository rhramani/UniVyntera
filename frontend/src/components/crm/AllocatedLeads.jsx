...
const AllocatedLeads = () => {
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("Allocated Leads");
  ...
  useEffect(() => {
    ...
    show,
    showViewModal || isWhatsappModalOpen || showDeadLeadModal || openModal,
  ]);
  ...
  }, [currentPage, filters, searchTerm, selectedFilter]);
  ...
  const changePage = async (page) => {
    ...
    try {
      ...
      const res = await dispatch(getLeadByAssignUserId(payload));
      setGetLeadData(res?.data);
      setTotalPages(res?.data?.totalPages || 0);
      setTotalRecords(res?.data?.totalLeads || 0);
    } catch (error) {
  ...
  const handleItemsPerPageChange = async (newItemsPerPage) => {
    ...
    if (canRead) {
      ...
      if (canRead) {
        dispatch(getLeadByAssignUserId(payload))
          .then((res) => {
            setGetLeadData(res?.data);
            setTotalPages(res?.data?.totalPages || 0);
            setTotalRecords(res?.data?.totalLeads || 0);
  ...
  const columns = [
    ...
    },
    {
      label: "Branch Lead Assign",
      key: "lead_assign_Branch",
      render: (item) => getBranchNameById(item.lead_assign_Branch),
    },
    {
      label: "Role",
      key: "lead_role",
      render: (item) =>
        getRoleNameById(item.lead_role, item.lead_assign_Branch),
    },
    {
      label: "Lead Assign",
      key: "lead_assign_name",
      render: (item) => item.lead_assign_name || item.lead_assign?.name || "-",
    },
    {
      label: "Remark",
      key: "remarks",
  ...
  return (
    <>
      ...
      <Pageheader
        mainheading="Allocated Leads"
        parentfolder="Leads"
        activepage="Allocated Leads"
      />
          ...
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0 mt-2">
              <div className="w-100 d-flex flex-wrap justify-content-end">
                {/* <div className="card-title">Allocated Leads</div>{" "} */}
            ...
            <Card.Body>
                    ...
                    {/* <div className="filter-item search-container">
                      <div className="position-relative">
                        <Form.Control
                          type="text"
                          placeholder="Search..."
                          className="filter-height ps-5 rounded-pill"
                          value={searchTerm}
                          onChange={(e) => {
              ...
              <ViewModal
                ...
                title="View Initial Lead Details"
              ...
              {activeView === "card" ? (
                <AllLeadsCard
                  ...
                  permissionName="Allocated Leads"
                  filters={filters}
                  selectedFilter={selectedFilter}
                  searchTerm={searchTerm}
                  currentPage={currentPage}
