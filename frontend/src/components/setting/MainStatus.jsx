...
import {
  ...
  getAllStudentStatus,
  deleteStudentStatus,
} from "../../redux/actions/Student/StudentStatus.action";
...
const MainStatus = () => {
  ...
  const fetchStudentStatuses = async (search) => {
    try {
      const res = await dispatch(getAllStudentStatus(search));
      if (res?.status === 200) {
  ...
  return (
    <>
      <Pageheader
        mainheading="Main Status"
        parentfolder="Application Statuses"
        activepage="Main Status"
      />
        ...
        <Col md={12} lg={12} xl={12}>
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              <div>
                <div className="card-title">Main Status</div>
              </div>
            </Card.Header>
            <Card.Body>
