...
const B2BLeadStatus = () => {
 const dispatch = useDispatch();
  ...
  const { canCreate, canRead, canUpdate, canDelete } =
    usePermissions("B2B Lead Status");
  ...
  const fetchLeadStatus = async (search) => {
    try {
      const res = await dispatch(getAllB2BLeadStatus(search));
      if (res?.status === 200) {
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        if (values.id && canUpdate) {
          ...
          const res = await dispatch(updateB2BLeadStatus(payload, values.id));
          if (res?.status === 200) {
            toast.success("B2B Lead Status updated successfully");
          }
        } else if (canCreate) {
          const { id, ...payload } = values; // Exclude id from payload
          const res = await dispatch(createB2BLeadStatus(payload));
          if (res?.status === 201) {
            toast.success("B2B Lead Status created successfully!");
          }
        }
  ...
  const handleDelete = async (item) => {
    try {
      toast.dismiss();
      const res = await dispatch(deleteB2BLeadStatus(item._id));
      if (res?.status === 200) {
        toast.success("B2B Lead Status deleted successfully");
      }
      if (canRead) {
        fetchLeadStatus(search);
      }
  ...
  return (
    <>
      <Pageheader
        mainheading="B2B Lead Status"
        parentfolder="Settings"
        activepage="B2B Lead Status"
      />
        ...
        <Col md={12} lg={12} xl={12}>
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              <div>
                <div className="card-title">B2B Lead Status</div>
              </div>
            </Card.Header>
            <Card.Body>
