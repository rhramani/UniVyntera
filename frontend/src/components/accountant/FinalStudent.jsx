...
const FinalStudent = () => {
  ...
  const fetchAllTotalAdmission = async (
    ...
    try {
      ...
      setTotalAdmissions(responseData?.data || []);
      setTotalPages(responseData?.totalPages || 0);
      setTotalRecords(responseData?.totalRecords || 0);
    } catch (error) {
      console.log("Error fetching reports students:", error);
      toast.error(
  ...
  const fetchAllStudentIds = async (
    ...
    try {
      ...
      const responseData = res?.data?.data;
      return responseData?.data?.map((lead) => lead?._id) || [];
    } catch (error) {
      console.log("Error fetching all student IDs for export:", error);
      return [];
    }
  };
  ...
  //       link.href = fileUrl;
  //       link.setAttribute("download", "final_students.csv");
  ...
  //         "Something went wrong while downloading reports."
  //     );
  ...
  const handleExport = async () => {
    try {
      setIsLoading(true);
      let ids = Object.keys(selectedItems).filter((id) => selectedItems[id]);
      let dataToExport = [];

      if (ids.length > 0) {
        dataToExport = totalAdmissions.filter((item) => ids.includes(item._id));
      } else {
        // const allData = await fetchAllStudentIds(
        //   search,
        //   filters.startDate,
        //   filters.endDate,
        //   filters.institute?.value,
      ...
      const rows = dataToExport.map((item) => {
        const instituteNames =
          item?.interestedCourseDetails?.length > 0
            ? item.interestedCourseDetails
                .map((course) => course?.institute?.instituteName || "N/A")
                .join(", ")
            : "-";
        const programNames =
          item?.interestedCourseDetails?.length > 0
            ? item.interestedCourseDetails
                .map((course) => course?.course?.programName || "N/A")
                .join(", ")
            : "-";
        const tuitionFee =
          item?.interestedCourseDetails?.length > 0
        ...
        return [
          ...
          item?.purposeDetails?.preferredCountry?.length > 0
            ...
            : "-",
          `${tuitionFee}`,
          item?.universityVerificationSent ? "Yes" : "No",
          item?.universityVerificationSent &&
          item?.universitySideConfirmation?.status
            ? "Yes"
            : "No",
          item?.universitytInvoiceGenerated?.status ? "Yes" : "No",
          item?.universityPaymentReceived?.status ? "Yes" : "No",
          item?.universityPaymentReceived?.b2bCommission?.paymentProcess ===
          "Paid"
            ? "Yes"
            : "No",
        ];
  ...
  return (
    <>
      <Pageheader
        mainheading="Final Student"
        parentfolder="Reports"
        activepage="Final Student"
      />
          ...
          <Card className="custom-card transcation-crypto">
            <Card.Header className="border-bottom-0">
              <div className="w-100 d-flex flex-wrap justify-content-end">
                {/* <div className="card-title">Final Student Report</div> */}
                {canRead && (
                  <>
                    <div className="d-flex flex-wrap gap-2">
                      ...
                      {totalAdmissions?.length > 0 && canDownload && (
                        <Button
                          variant="primary"
            ...
            <Card.Body>
              ...
              >
                ...
                >
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {totalAdmissions?.length > 0 ? (
                      totalAdmissions.map((item, index) => {
              ...
              {totalPages > 1 && totalAdmissions.length > 0 && (
