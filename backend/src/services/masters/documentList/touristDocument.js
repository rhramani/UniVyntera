...
const touristDocumentServices = {
  ...
  const upsertCountryDocuments = async (countryName) => {
    const existing = await TouristDocument.findOne({
    ...
    // Create new entry
    const newEntry = await TouristDocument.create({
      country: countryName,
      documents,
      created_by: userId,
  ...
  update: async (countryDocId, data, userId, userName) => {
    ...
    const existing = await TouristDocument.findById(countryDocId);

    if (!existing) {
      throw { status: false, message: "Tourist Document not found." };
    }

    const duplicate = await TouristDocument.findOne({
    ...
    const updatedEntry = await TouristDocument.findByIdAndUpdate(
      countryDocId,
      {
        country: country,
        documents,
  ...
  delete: async (deleteId) => {
    const doc = await TouristDocument.findByIdAndDelete(deleteId);
...
module.exports = touristDocumentServices;
