const workPermitDocument = require("../../../../model/masters/documentList/workPermitDocument");
...
const workPermitDocumentServices = {
  ...
  const upsertCountryDocuments = async (countryName) => {
    const existing = await workPermitDocument.findOne({
    ...
    // Create new entry
    const newEntry = await workPermitDocument.create({
      country: countryName,
      documents,
      created_by: userId,
  ...
  return {
    status: true,
    message: "Tourist documents created/updated successfully.",
    data: result,
  };
},
  ...
  update: async (countryDocId, data, userId, userName) => {
    ...
    const existing = await workPermitDocument.findById(countryDocId);

    if (!existing) {
      throw { status: false, message: "Tourist Document not found." };
    }

    const duplicate = await workPermitDocument.findOne({
    ...
    const updatedEntry = await workPermitDocument.findByIdAndUpdate(
      countryDocId,
      {
        country: country,
        documents,
  ...
  delete: async (deleteId) => {
    const doc = await workPermitDocument.findByIdAndDelete(deleteId);
...
module.exports = workPermitDocumentServices;
