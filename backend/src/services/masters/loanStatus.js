...
const loanStatusServices = {
    create: async (data, userId, userName) => {
        const { name, color } = data;

        const nameExist = await LoanStatus.findOne({ name });
        ...
        const colorExist = await LoanStatus.findOne({ color });
        if(colorExist){
            throw{
                status: false,
     ...
     getAll: async (searchText = "") => {
        ...
        const allStatus = await LoanStatus.find(query)
        ...
        getById: async (id) => {
          const status = await LoanStatus.findById(id);
          if (!status) {
         ...
         update: async (id, data, userId, userName) => {
            ...
            if (name) {
              const nameExist = await LoanStatus.findOne({
                name,
                _id: { $ne: id }
              });
              if (nameExist) {
            ...
            if (color) {
              const colorExist = await LoanStatus.findOne({
                color,
                _id: { $ne: id }
              });
              if (colorExist) {
            ...
            const updatedStatus = await LoanStatus.findByIdAndUpdate(
              id,
              {
                ...data,
                updated_by: userId,
            ...
            delete: async (id) => {
              const deleted = await LoanStatus.findByIdAndDelete(id);
              if (!deleted) {
                throw {
                  status: false,
...
module.exports = loanStatusServices;