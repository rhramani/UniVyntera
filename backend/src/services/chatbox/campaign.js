...
const { getTemplates } = require("../../services/chatbox/template");
...
exports.getCampaignReport = async () => {
  return await CampaignLogModel.aggregate([
    {
      $lookup: {
        from: "chatboxcampaigns", // collection name in MongoDB (lowercase + plural by convention)
...
exports.sendSingleMessage = async (
  ...
  // Step 1: Get template
  const template = await getTemplates();
  console.log("---template----",template.data.map((v) => v.name))
  let templateDetails;
  ...
  try {
    response = await axios.post(
      `${config.chatboxUrl}/${config.chatboxChoneNumberId}/messages`,
      data,
      { headers }
    );
  } catch (error) {
